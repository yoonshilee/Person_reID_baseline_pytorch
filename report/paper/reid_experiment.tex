...
In this work, we reproduce a baseline model on Market-1501~\cite{zheng2015scalable} and then run controlled variants on DukeMTMC-reID.
We focus on (1) baseline reproduction, (2) domain shift across datasets, (3) the effect of changing backbones and loss functions, and (4) failure-case analysis and safety reflection.
...
\begin{figure}[t]
  ...
  \caption{Two DukeMTMC-reID failure cases (Top-10 retrieval visualization). Top: Case-1 (query index 1883, label 4315). Bottom: Case-2 (query index 67, label 51).}
